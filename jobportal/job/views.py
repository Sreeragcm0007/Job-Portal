...
def recruiter_signup(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            Recruiter.objects.create(user=user, mobile=c, image=i, gender=g,company=c, type="recruiter",status="pending")
            error = "no"
        except:
            error = "yes"
    d = {'error': error}
...
def add_job(request):
    ...
    if request.method == 'POST':
        ...
        recruiter =  Recruiter.objects.get(user=user)
        try:
           Job.objects.create(recruiter= recruiter,start_date=sd,end_date=ed,title=jt,salary=sal,image=l,description=des,skills=skills,experience=exp,location=loc,creationdate=date.today())
           error = "no"
        except:
             error = "yes"
    d = {'error': error}
